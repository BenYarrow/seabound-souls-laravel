...
- [ ] **Dark-mode eyeball** the new tag surfaces (`Blog/Tag.tsx`, index tag bar, post chips) in both themes — they reuse `Blog/Index.tsx`'s raw colour utilities, so they inherit the same light-only state as the rest of the blog. Will be subsumed by the dark-mode token-layer sweep below. (Visual check was deferred at merge — browser preview tooling was unresponsive.)
