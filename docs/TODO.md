...
- [ ] **Visual eyeball** the tag surfaces (`Blog/Tag.tsx`, `Blog/Tags.tsx` hub, `TagMasthead` gradient fallback, index tag bar, post chips) — the gradient fallback hero in particular was never seen (browser tooling was down at build time). Also check dark mode: in both themes — they reuse `Blog/Index.tsx`'s raw colour utilities, so they inherit the same light-only state as the rest of the blog. Will be subsumed by the dark-mode token-layer sweep below. (Visual check was deferred at merge — browser preview tooling was unresponsive.)
