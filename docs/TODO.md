...
- [ ] **Visual eyeball** the tag surfaces (`Blog/Tag.tsx`, `Blog/Tags.tsx` hub, the `TagMasthead` gradient fallback hero, index tag bar, post chips) in both light and dark — the gradient fallback hero in particular was never seen (browser preview tooling was unresponsive at build time). These reuse `Blog/Index.tsx`'s raw colour utilities, so they inherit the same light-only state as the rest of the blog; the dark-mode token-layer sweep below will subsume that.
