...
## Right now
- **Test harness working** (`RefreshDatabase` + `SCOUT_DRIVER=null`; Scout tests opt into the `collection` engine).
- **Blog + SpotGuide + Destinations + Search slices fully tested + commented** — suite at **29 tests, 224 assertions**.
...
## Next action
1. Continue the test/comment sweep across public controllers — **Contact next** (index + store, with validation + mocked mail/reCAPTCHA), then Pages, Homepage.
