...
## Right now
- **Test harness is working** (`RefreshDatabase` + `SCOUT_DRIVER=null`) — was previously non-functional.
- **Blog slice fully tested + commented** (7 tests, 60 assertions) — the template for the wider sweep.
...
## Next action
1. Fan out the blog test/comment pattern across public controllers — **SpotGuide first** (richest page: controller + model + relationships + weather), then Destinations, Search, Contact (incl. form validation + mocked mail/reCAPTCHA), Pages, Homepage.
2. Then models + helpers (relationships, scopes, weather-data transforms), Filament last.
