...
- **Weather fetch triggers** — shared `WeatherFetcher` service (one fetch path for the weekly command + both jobs); auto-fetch on spot-guide create (`FetchSpotWeatherJob`); dashboard "Fetch all weather" button (`FetchAllWeatherJob` + in-app bell notification); coordinates now required + range-validated. Needs a queue worker running. Suite **80 tests, 466 assertions**.
